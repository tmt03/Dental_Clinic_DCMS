...
public class EditProfileServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (!email.equals(userSS.getEmail())) {
                if (dao.existedEmail(email)) {
                    request.setAttribute("ERROR", "Email already exists");
                } else {
                    dao.updateProfile(displayName, email, address, mobile, others, image, dob, username);
                    request.setAttribute("STATUS", "Update Profile Successful");

                    User user = dao.check(userSS.getUsername(), userSS.getPassword());
